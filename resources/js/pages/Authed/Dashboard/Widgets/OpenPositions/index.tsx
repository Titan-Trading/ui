...
export default function OpenPositions({ }: OpenPositionsProps) {
	...
	useEffect(() => {
		...
		getMetrics('open-positions').then((res: any) => {
		  console.log(res);
	...
	const openPositions = positions.map((p: any) => {
		return (<tr key={'open-position-' + p.id}>
            <td>
			...
			<td>
                <Button size="xs" color={p.profit_loss >= 0 ? 'teal' : 'red'}>
                    EXIT
                </Button>
			</td>
		</tr>);
